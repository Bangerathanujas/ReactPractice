...
var GmailBox = React.createClass({
  ...
  getEmailByLabels:function(label){
    var accessToken = localStorage.getItem('gToken');
    $.ajax({
      url:'https://www.googleapis.com/gmail/v1/users/me/messages?labelIds='+label+'&maxResults=10&key={AIzaSyDoBcnEkXMkHak7dm2dS_1WqjhfOGCFl8M}',
      dataType:'json',
      type:'GET',
      async:false,
      beforeSend: function (request)
      ...
      success:function(data1){
        ...
        for (var i = 0; i < temp.length; i++) {
          ...
          msg.push(this.SpecificMessages(temp[i].id));
          console.log(msg);



        }
        this.setState({allMessages:msg});
  ...
  SpecificMessages:function(id){
    ...
    var a3=$.ajax({
      ...
      success:function(data2){
        ...
        //this.setState({allMessages:data2});
        // this.state.allMessages.push(data2);
        //this.seState({allMessages:data2});
      }.bind(this),
      error:function(xhr,status,err){
        console.error(err.toString());
      }.bind(this)
    });
    return a3.responseJSON;
  },
  ...
  render:function()
  {
    ...
    if(loadedData){
      ...
      leftPanel =  <Labels p1={this.state.allLabelsData} p3={this.getEmailByLabels} ></Labels>
